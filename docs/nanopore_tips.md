...
以下、MinIONの使い方を解説する。GridION、PromethIONなどは扱い方が変わるので注意。また、基本的に植物サンプルのゲノムを決定することを想定した解説になっている。
## 必要な設備・試薬
...
### フローセルの再利用
Wash Kitを使うことでフローセルを再利用可能。植物の場合は2回くらいまでなら洗って再利用できる。昆虫や真菌の場合は（多糖が少ないため？）もっと再利用できるらしい。WashしたらPoreの状況を確認するが、Activateした方が正確な値が出る。1-2回目ならどうせシーケンスするのでActivateしておいてもいいかも。公式サイトでは再利用を含めて1枚のフローセルから最大50Gbpの出力が得られることになっているが、植物の場合は1枚のフローセルから10Gbpくらいの配列が得られれば成功、20Gbpくらいのデータが得られれば大成功くらいの気持ちでいたほうが良い。
