...
スパコンは基本的にLinux経由で動かすので、コマンドなどで分からないことがあったらLinux関連のページを調べると良いかもしれない。Linuxの基本的な使いかたは[ここ](docs/lnx.md)。また、管理者権限が無いので、`sudo`は実行できない。動かしたいソフトやコマンドがスパコン環境内に無い場合は、ソースをダウンロードして使うのが基本。
