...
- [DNAの抽出](#dnaの抽出)
- [RNAの抽出](#rnaの抽出)
- [保存状態の悪いサンプルからのDNA抽出](#https://github.com/k-cgi/k-cgi.github.io/blob/main/docs/dna_tips.md#%E4%BF%9D%E5%AD%98%E7%8A%B6%E6%85%8B%E3%81%AE%E6%82%AA%E3%81%84%E3%82%B5%E3%83%B3%E3%83%97%E3%83%AB%E3%81%8B%E3%82%89%E3%81%AEdna%E6%8A%BD%E5%87%BA%E5%8F%A4%E3%81%84%E3%82%A2%E3%83%AB%E3%82%B3%E3%83%BC%E3%83%AB%E3%81%8C%E8%96%84%E3%81%84%E8%85%90%E3%82%8A%E3%81%8B%E3%81%91%E3%81%AA%E3%81%A9)
- [コンタミの防止](#コンタミの防止)
- [RNAの抽出](#rnaの抽出)
- [DNA、RNAの保存](#dna、rnaの保存)
...
基本的に普通にDNAを抽出するが、保存状態の悪いサンプルに含まれるDNAは断片化し、PCRに利用可能なDNA量が減っている。そのため、保存状態のいいサンプルやDNAが混入してしまうとそちらのDNAの絶対量が多くなってしまい、PCR等した際にそちらのDNAばかり検出されてしまう。これを防ぐためにはコンタミに配慮した実験が必要。また、抽出の際にサンプルをよくつぶす、PCRをする場合にはターゲット領域を200bp以下（場合によっては50bp以下）に設定するなどの配慮をする。DNAが抽出されているのにPCRが上手くいかない場合（バンドが出ない、他の種のDNAが増えるなど）は抽出されているDNAが断片化していることを疑う。乾燥標本などでは、カビのDNAが抽出されている例もあるようだ。

## コンタミの防止
基本的なコンタミの防止策を徹底する。チップやチューブは確実に使い捨てること、ピペッティングの際に飛沫が飛ぶことに留意すること、チューブのキャップ開ける前には必ずスピンダウンすることなど。また、保存状態の悪いサンプルの場合、他のサンプルで使用したピペットを使うとそこからPCR産物がコンタミし、PCRがうまくいかないことがある。このようなサンプルを扱う場合は、PCR産物を扱わない部屋やピペットを用意し、フィルターチップを利用してDNAの抽出-PCRの準備を行う。

## RNAの抽出
RNAはDNAよりも分解されやすいため、実験に配慮が必要。特に、人間の体から分泌されるRNaseにより容易に分解されるため、手袋着用、RNase Awayなどの試薬でRNase汚染を防ぐ。また、専用の実験台やピペット、フィルターチップを使う。サンプルの質も重要で、生きたサンプルを使うか、RNAの分解を抑止する試薬（RNA later等）で保存されたサンプルを使うこと。

### Maxwell 16 LEV Plant RNA Kit
つぶしたサンプルを入れ、スイッチを押すだけでOK。楽だが高い。1サンプルあたり850円程度。準備はサンプルあたり10分程度。稼働に50分程度かかり、サンプルが1増えるごとに1分増える印象。抽出されたDNAは50Kbp以下？少なくとも100Kbpとかではない。抽出されたRNAの質・量ともに高く、RNA-seqに利用可能。
...
- 植物の生葉、5mm-10mm程度
- 植物の茎、5mm-10mm程度
- 植物の根、5mm-10mm程度、ただし濃度が低く、不純物が多い。

#### 手順
説明書に書いてある通りに行う。サンプルの破砕はビーズ破砕等で入念に行う。また、多糖が多い部位（根など）では抽出量が減り、不純物が増える。

#### 抽出物に関して
不純物は少ない。濃度測定する場合、吸光度による測定でも十分。薄めてRNA-seqに利用可能である。


## DNA、RNAの保存
RNAはRNaseによる分解を避けるため、-80℃で保存する。DNAは長期保存するなら-30℃、頻繁に使うサンプルなら冷蔵で良い。冷凍と融解を繰り返すと、DNAが断片化しすぐにPCRがかからなくなるので注意。2000bp程度の配列でも10回程度の融解・凍結ですぐ増えなくなる。冷蔵保存する場合、干からびないように注意。干からびた場合は水を足して1日置いておくと使えるようになることもある。
